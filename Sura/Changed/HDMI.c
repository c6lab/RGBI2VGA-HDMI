...
//mod Panika %) //>>
extern cap_set_t capture_setings;
//mod Panika %) //<<
...
//mod Panika %) //>>
    if (capture_setings.p_clk_mode == SELF_SYNC_MODE)
    {
        size_t fear_size = scr_W - V_BUF_W;
        if (fear_size)
        {
            for(int i = fear_size; i--;)
            {
                 *activ_buf_l++ = palette[0];
                 *activ_buf_l++ = palette[1];
            }
        }
    }
//mod Panika %) //<<
